...
function validateOpts(opts: Opts): void {
  ...
  if (opts.url) {
    try {
      ...
      errors.push(`Invalid URL: "${opts.url}". Must be a valid URL (e.g., http://localhost:5173)`);
    }
  }
  ...
  if (opts.viewport) {
    ...
    if (!vpMatch) {
      errors.push(`Invalid viewport: "${opts.viewport}". Must be WIDTHxHEIGHT (e.g., 1280x800)`);
    } else {
...
async function snapshot(opts: Opts): Promise<void> {
  ...
  try {
      ...
      (window as any).__snapshot = {
        ...
        replaceCssUrls: (
          ...
          for (const r of sorted) {
            cssText =
              cssText.substring(0, r.start) + "url('" + r.dataUri + "')" + cssText.substring(r.end);
          }
          return cssText;
        },
      };
            ...
            try {
              ...
              if (doc && doc.body) {
                ...
                doc
                  ...
                  .forEach((l) => styles.push((l as HTMLLinkElement).outerHTML));
      ...
      for (const { frame } of childFrames) {
        try {
          ...
          const frameStyles = await frame.evaluate(() => {
            ...
            document
              .querySelectorAll('link[rel="stylesheet"]')
              .forEach((l) => stylesList.push((l as HTMLLinkElement).outerHTML));
            return stylesList;
          });
            ...
            await parent.evaluate(
              (url, bodyHtml, styles, bodyAttrs, htmlAttrs) => {
                ...
                for (const iframe of iframes) {
                  ...
                  if (
                    cleanIframeSrc &&
                    (url.includes(cleanIframeSrc) || cleanIframeSrc.includes(url))
                  ) {
                    const wrapper = document.createElement("div");
                    wrapper.className = "ac-iframe-inlined-wrapper";
    ...
    stats.cssUrls = await page.evaluate(async (inlineFonts) => {
      const { toDataUri, processInBatches, extractCssUrls, replaceCssUrls, CONCURRENCY } = (
        window as any
      ).__snapshot;
      let count = 0;
    ...
    const additionalStats = await page.evaluate(async () => {
      ...
      // --- <video poster="..."> ---
      const videos = Array.from(document.querySelectorAll("video[poster]")) as HTMLVideoElement[];
      ...
      // --- <object data="..."> ---
      const objects = Array.from(document.querySelectorAll("object[data]")) as HTMLObjectElement[];
    ...
    const additionalTotal = stats.svgImages + stats.videoPoster + stats.favicons;
    console.log(
    ...
    console.log(`   ${additionalTotal} additional resources (SVG/video/favicon)`);
